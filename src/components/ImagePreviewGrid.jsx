...
const ImagePreviewGrid = ({ files, onRemove, onSelect }) => {
    console.log("from image preview: ", files);
    return (
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {files.map((img, index) => (
                <div
                    key={index}
                    className="relative group bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden transition transform hover:scale-105 hover:shadow-lg duration-300 cursor-pointer"
                >
                    <img
                        src={img.preview}
                        alt={`preview-${index}`}
